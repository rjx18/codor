...
All notable changes to Wireroom are documented here. This project follows semantic versioning once
...
- Local-first room switchboard with SQLite history, durable routing, compact run evidence, brakes,
...
- Responsive installable web PWA with paired browser identity, sealed Web Push, desktop room and run
  inspector, settings, relay disclosure, and a read-only ledger graph.
- Ed25519/X25519 device identity, single-use pairing, room-key rotation, Noise-authenticated private
...
- Bearer, pairing, peer, and browser authority fail closed; revocation rotates room keys and purges
...
- Secrets and raw model events are redacted before durable display, push previews are fixed-size
  authenticated ciphertext, and external bridges require explicit room opt-in.
