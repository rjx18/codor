...
| Additional harness adapters (Copilot CLI, OpenCode, Gemini, Pi, …) | paseo's adapter set as the *behavioral reference* (flags, session stores, event quirks — AGPL forbids copying code, not learning from it); ACP adapters where they exist (Apache/MIT); each harness's first-party headless docs | pattern / depend | our adapter surface is 4 functions over CLI JSONL, so each new harness is a small file — the reference mostly saves discovery time |
