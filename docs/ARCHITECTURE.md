...
| Additional harness adapters (Copilot CLI, OpenCode, Gemini, Pi, …) | paseo's adapter set as the *behavioral reference* (flags, session stores, event quirks — AGPL forbids copying code, not learning from it); ACP adapters where they exist (Apache/MIT); each harness's first-party headless docs | pattern / depend | our adapter surface is 4 functions over CLI JSONL, so each new harness is a small file — the reference mostly saves discovery time. **AGPL scope note:** pasting paseo code into the switchboard relicenses the *daemon* to AGPL (copyleft attaches to the program it's compiled/linked into — the closed phone apps, being separate programs over a network API, were never the issue). Recorded fallback if an adapter proves gnarly: run paseo code as a **quarantined sidecar process** (own repo, AGPL, talks to the MIT switchboard over a socket) — arms-length separation keeps the core MIT. Prefer writing the small adapter; use the sidecar only if truly needed |
