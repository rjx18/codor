...
- `packages/adapters/claude-code`: Agent SDK driver (resume, stream, ask-user, canUseTool →
  approval cards).
...
voice, trip and release a hop budget — while the switchboard sees no plaintext leave the tailnet
...
- Multi-human rooms: presence, per-human notification routing.
