...
Wireroom stores a role on every human room member and enforces it at the
switchboard. The open protocol and self-hosted switchboard own this enforcement;
the hosted Relay is not an authorization oracle and never receives room
plaintext.
...
| --- | --- |
| `observer` | Read rooms, history, run evidence, ledger notes, and their own inbox state. |
...
| `owner` | Admin access plus keys, paired devices, roles, and room lifecycle. |

The default self-hosted installation has one authenticated operator. Every new
room seeds that human as `owner`; its bearer and locally private Unix socket
...
Roles govern human control of a room. Agent and extension execution is not
...
when it hosts an agent in that room and the attributed writer is either that
...
still evaluates the role before it mutates room state. A hosted service cannot
