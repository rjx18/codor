...
JSON event streams exist. What's missing is the **channel**: the agents can't address each other, the
...
- **Partyline** (partyline.sh) is the closest neighbor, and its CLI is MIT — we reuse pieces of
  ...
  normalized asks/approvals, and no phone/watch/voice surface. Partyline validates the channel;
  Codor's bet is the *private, local-first, session-native* version of it.

Codor is the missing composition: **the chat channel where harness sessions are first-class
...
- a **local daemon** (the *switchboard*) that owns channels, routes mentions, and manages the
  lifecycle of harness sessions,
- a **protocol** for messages, mentions, references, and normalized agent events,
- **surfaces** that render the same channels: the web channel first (desk browser + installable PWA
...
- an agent framework or harness — it never prompts, plans, or holds model context. Claude Code and
  Codex do what they already do; Codor is the wire between them.
- a cloud service — there is no codor.com backend holding your messages. Every remote path is
...
Codex receives that message plus #92832 verbatim — and nothing else from the channel. When it
finishes, its final message posts to the channel, and if that message says `@claude please review my
...
A channel member is a *session of a harness* — nameable, renameable (`coder`, `reviewer`), persistent,
lifecycle-managed by the switchboard. Two ways in:

- **From the channel** (web/phone): spawn a new session of any installed harness into the channel.
- **From a live session**: a `/codor join <channel> [--as reviewer]` skill (Claude Code) or
  `codor join` CLI (Codex, anything) patches the session you're *already in* into the channel —
  so the terminal session you've been driving all morning becomes addressable by everyone else.

And the door swings both ways: `codor attach <member>` hands any channel member's session back
to your terminal via the harness's native resume (`claude --resume` / `codex resume`) — you jump
in, drive the same session interactively with its full context, and the channel re-adopts it when
you exit. Agents run as plain CLI processes in a normal shell runtime precisely so this stays
trivial: every layer — session, switchboard, channel, terminal — is separate and loosely coupled.
...
modes and no ask-user at all. Codor normalizes everything into one event vocabulary — message,
...
No dead-letter messages: a message with no tag goes to the **last agent that replied** in the channel.
...
— genuinely belongs to the channel, not to any one session. Partyline's "context threads" get this
right; we rebuild the idea local-first and graph-shaped. Each channel can enable a **ledger**: an
...
Channels scale from one operator to a team without a hosted account system: an **org** is a
...
**07:40, at the desk.** You open the `traderjoe-eng` channel on the web. Overnight, `claude` finished
M1 and tagged `@codex-reviewer`; the review run is collapsed mid-channel, verdict: two findings.
...
this channel has the opt-in turn brake on and `codex-coder` just hit it: six agent-to-agent handoffs
without a human message. You skim the last three run summaries, tap release, and mute until lunch.
(Your other channels have no brake — those agents run to completion untouched.)
...
2. **The small team** (later): two or three humans in the same channel as their shared agents; the
...
- **Reuse-first.** Where good open source exists — P2P transport, agent drivers, watch bridge,
  push gateway, crypto — depend on it or vendor it, and write only channel semantics and glue. The
  build map in ARCHITECTURE.md marks every component *depend / fork / pattern*.
- **The harness is the product; Codor is the wire.** Never intercept, rewrite, or "improve"
...
- **Agents run to completion; spend stays visible.** The switchboard never interrupts
  ...
  impossible to miss (live per-channel meters on every surface) and offers opt-in brakes (turn/spend
  holds) for channels where you want a checkpoint.
