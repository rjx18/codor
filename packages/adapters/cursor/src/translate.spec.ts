...
describe('cursor stream-json translation', () => {
  ...
  it('emits exactly one run.completed when the stream carries two results', () => {
    // cursor-agent can emit a second result event for the same turn. The daemon
    // finalizes on the first completion, so a second one lands against a turn
    // that has already settled — it must never be produced.
    const translator = createTurnTranslator();
    const first = translator.push(JSON.stringify({
      type: 'result', subtype: 'success', is_error: false, result: 'Done.',
      usage: { inputTokens: 3, outputTokens: 4 },
    }));
    expect(first).toEqual([{
      type: 'run.completed', status: 'completed', final_text: 'Done.',
      usage: { input_tokens: 3, output_tokens: 4 },
    }]);

    const second = translator.push(JSON.stringify({
      type: 'result', subtype: 'success', is_error: false, result: 'Done.',
      usage: { inputTokens: 3, outputTokens: 4 },
    }));
    expect(second).toEqual([]);

    // A duplicate that would have FAILED the turn must not reclassify it either.
    expect(translator.push(JSON.stringify({
      type: 'result', subtype: 'error', is_error: true, result: 'late failure',
    }))).toEqual([]);

    // And EOF after a settled turn adds nothing.
