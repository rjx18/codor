  ...
  return {
    ...
    push(line: string): WireEvent[] {
      ...
      switch (event.type) {
        ...
        case 'result': {
          // A stream can carry more than one result event. The daemon finalizes
          // on the first `run.completed`, so a second one arrives against an
          // already-settled turn — the flag below was set here but never read,
          // which is exactly how that second frame escaped.
          if (terminal) return [];
          terminal = true;
