...
them: it runs `opencode models` as an async child with a fixed argument vector (no shell),
a 20 s budget and capped output, and reports `source: 'discovered'`. Nothing is hardcoded.
The probe never occupies the event loop, and the child is confirmed terminated on every
...
per harness (`models_error`) so the dialog offers a retry instead of reporting no models;
a prior catalog is kept when one exists. The dialog falls back to its `Custom…` escape
