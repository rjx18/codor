...
function writeBlockedTranscript(input: {
  ...
  const lines = [
    ...
    '- PASS: separate turn-brake channel held and released the same delivery.',
    '- PASS: history paging, search, and #N permalinks (Playwright; recorded in test summary).',
    '',
    '## Live attempt',
    '',
    'Models were pinned to Claude `haiku` and Codex `gpt-5.4-mini`; review-channel turn and',
    ...
    `The CLI joined active implementer session \`${input.joinSession}\` into ` +
      `channel \`${input.reviewRoom}\` as \`@implementer\` with mirrored custody.`,
    '',
    ...
    `2. Invoke \`/codor\`, choose \`join\`, channel \`${input.reviewRoom}\`, handle \`implementer\`.`,
    ...
    '## Brake channel',
    '',
    `Human message #${String(input.brakePost)} started a separate channel with \`turn_brake=1\`.`,
  ];
...
function writeTranscript(input: {
  ...
  const lines = [
    ...
    'Review-channel brakes: turn off, spend off. Live prompts were each issued once.',
    '',
  ...
  lines.push(
    ...
    '1. Open the existing Claude or Codex TUI session in its terminal.',
    '2. Invoke `/codor`, choose `join`, channel `m1-review`, and handle `implementer`.',
    '3. Confirm the channel member appears with mirrored custody.',
    ...
    'live Codex channel member. The automated acceptance used a short deterministic interactive child instead',
    'of attempting to drive the fullscreen native TUI from a non-interactive test runner.',
    '',
    '## Brake channel',
    '',
    `Human message #${String(input.brakePost)} started the separate turn-brake=1 channel.`,
  );
  for (const message of brakeMessages) {
    const author = brakeMembers.get(message.author) ?? message.author;
    const status = message.run ? ` [${message.run.status}]` : '';
