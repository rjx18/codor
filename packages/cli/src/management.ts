...
export function classifyManagementError(error: unknown): ManagementError {
  ...
  const worktreeConflictFailure = /worktree target already exists|worktree target is (?:dirty|missing|mismatched|locked|unavailable|prunable)|worktree path is not a discovered candidate|primary checkout is registered as main|only an available, unlocked worktree can be adopted|primary checkout is unavailable|worktree target overlaps (?:an existing checkout|git administrative state)|primary checkout has no commit head|created worktree belongs to a different repository|created worktree could not be rediscovered|worktree was created but registration failed|worktree repository (?:is not registered|is unavailable)|worktree belongs to a different or unavailable repository|registered worktree is missing or mismatched|registered worktree identity does not match git|worktree path is missing|worktree is locked, prunable, or unavailable|worktree must be clean before removal|worktree cannot be removed|worktree is unavailable|worktree alias is already in use|worktree alias .* ambiguous|worktree .* is not active|primary worktree cannot be removed|the main alias is reserved|a room may register only one git repository|local branch already exists|could not verify local branch|no active worktree|only an active secondary worktree can be removed|only an active secondary worktree can be unregistered|live child runtime|unresolved work|room has no existing repository cwd/;
