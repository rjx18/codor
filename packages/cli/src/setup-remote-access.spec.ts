...
describe('runRemoteAccess', () => {
  ...
  it('keeps a Serve failure onscreen and requires an explicit local confirmation before advancing', async () => {
    const shown: Menu[] = [];
    let attempts = 0;
    const result = await runRemoteAccess(base({
      configureServe: () => { attempts += 1; throw new Error('serve: not logged in'); },
      choose: scripted(['configure', 'here', 'confirm'], shown),
    }));
    ...
    // A separate confirmation was required before going local.
    expect(shown[2]!.message).toContain('Continue on this computer only?');
    expect(result).toEqual({ access: 'localhost', endpoint: 'http://127.0.0.1:8137', summary: 'This computer (remote access deferred)' });
  });

  it('shows the operator recovery for a Linux permission error and does not advance automatically', async () => {
    const shown: Menu[] = [];
    const result = await runRemoteAccess(base({
      configureServe: () => { throw new Error('Tailscale Serve command failed: access denied; you must have operator permission'); },
      choose: scripted(['configure', 'here', 'confirm'], shown),
    }));
    const failureScreen = shown[1]!.message;
  ...
  it('lets the local confirmation bounce back to another Serve retry', async () => {
    let attempts = 0;
    const result = await runRemoteAccess(base({
      configureServe: () => {
        attempts += 1;
      ...
      // Fail once, choose Continue, then change your mind (retry) at the confirm.
      choose: scripted(['configure', 'here', 'retry']),
    }));
    expect(attempts).toBe(2);
