...
describe('group round payload composition', () => {
  ...
  it('names the paid next round and automatic terminal-peer wait outcome', () => {
    ...
    expect(payload).toContain(
      'Use <ACK_OK> as your entire onward response only when no action and no answer are needed',
    );
    expect(payload).toContain('never append it after doing work or as a sign-off');
    expect(payload).not.toContain('If no substantive onward response is needed');
  });
