...
for(const hosted of [false,true])for(const long of [false,true]){
  test(`${hosted?'hosted':'direct'} ${long?'long':'brief'} outage keeps local sends independent of readiness`,async({page})=>{
    ...
    // Combined P1/P3 local admission must not enable P4 targeted mutation or
    // resurrect P2's removed synchronization skeleton.
    expect(await page.evaluate(() => (window as any).__codor.actForRoom('ops', { act: 'archive_room' }, 'offline-archive'))).toBe(false);
