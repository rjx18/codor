...
// harn:assume web-glass-theme-accessible-modes ref=axe-accessibility-gate
// The gate that keeps the six violation classes out. It runs the same twenty-five states
// in both themes that found them: a gate that only visits the states nobody broke is not
// a gate — two of those six lived exactly where nobody had looked.
const THEMES = ['light', 'dark'] as const;

async function scan(page: import('@playwright/test').Page): Promise<string[]> {
  // Let transitions settle. axe composites what it sees, so a panel caught mid-fade
  // reports the blend as a contrast failure that the settled surface does not have.
  await page.waitForTimeout(350);
  const { default: AxeBuilder } = await import('@axe-core/playwright');
  const { violations } = await new AxeBuilder({ page }).analyze();
  return violations.map((v) => `${v.id} [${String(v.impact)}] ${v.nodes.map((n) => n.target.join(' ')).join(', ')}`);
}

async function open(page: import('@playwright/test').Page, theme: string, url = ROOM): Promise<void> {
  await page.addInitScript((t) => { localStorage.setItem('codor-theme', t); }, theme);
  await page.goto(url);
}

test('every route and interactive state is axe-clean, in both themes', async ({ page }) => {
  test.slow();
  const found: string[] = [];
  const record = async (state: string, theme: string): Promise<void> => {
    for (const v of await scan(page)) found.push(`${theme}/${state}: ${v}`);
  };

  // Room-mutating states first, while the daemon is fresh: a held delivery does not
  // survive a full sweep before it, and a bridged channel is sticky, so once enabled it
  // paints every later surface — which only widens the coverage.
  for (const theme of THEMES) {
    await control('/hold', { body: '@alpha resume the pocket flow' });
    await open(page, theme);
    await expect(page.getByTestId('hold-banner')).toBeVisible();
    await record('room:hold', theme);
  }
  await control('/bridge-enable');
  for (const theme of THEMES) {
    await open(page, theme);
    await expect(page.getByTestId('bridged-room-banner')).toBeVisible();
    await record('room:bridged', theme);
  }

  for (const theme of THEMES) {
    const handle = `t${theme}`; // unique per theme: a collision would suffix and break the flow

    await open(page, theme);
    await expect(page.getByTestId('connection')).toHaveAttribute('title', 'connected');
    await record('room:baseline', theme);

    await page.getByTestId('toggle-message-search').click();
    await expect(page.getByTestId('message-search')).toBeVisible();
    await record('room:search', theme);
    await page.keyboard.press('Escape');

    await page.getByTestId('composer-input').fill('@a');
    await expect(page.getByTestId('mention-popup')).toBeVisible();
    await record('room:mention-popup', theme);
    await page.getByTestId('composer-input').fill('');

    await page.getByTestId('spawn-agent').click();
    await expect(page.getByTestId('spawn-dialog')).toBeVisible();
    await record('dlg:spawn', theme);

    // Spawn, kill and offer to remove — the only path to the removal alertdialog.
    ...
    await expect(page.getByTestId(`remove-${handle}-confirm`)).toBeVisible();
    await record('dlg:remove-member', theme);
    await page.keyboard.press('Escape');

    await page.getByTestId('create-room').click();
    await expect(page.getByTestId('create-room-dialog')).toBeVisible();
    await record('dlg:create-channel', theme);
    await page.getByTestId('browse-folders').click();
    await expect(page.getByTestId('folder-picker')).toBeVisible();
    await record('dlg:folder-picker', theme);
    await page.keyboard.press('Escape');

    await page.evaluate(() => window.__codor.disconnect());
    await record('room:offline', theme);
    ...
    await record('room:read-only', theme);
    ...
    await expect(page.getByTestId('ledger-graph-surface')).toBeVisible();
    await page.getByTestId('ledger-node-launch-plan').click();
    await record('ledger:docked', theme);
    await page.setViewportSize({ width: 1200, height: 900 });
    await page.reload();
    await page.getByTestId('ledger-node-launch-plan').click();
    await record('ledger:overlay', theme);
    await page.setViewportSize({ width: 1440, height: 900 });
    ...
    await record('ledger:note-dialog', theme);
    await page.keyboard.press('Escape');
